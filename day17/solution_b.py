...
def dijkstra(graph: list[str]) -> int:
    ...
    n_cols = len(graph[0])
    start_items = [
        Item(coord=Coord(0, 0), direction=Direction.right, consecutive=0),
        Item(coord=Coord(0, 0), direction=Direction.down, consecutive=0),
    ]
    ...
    heat_losses: dict[Item, int] = {start_item: 0 for start_item in start_items}
    ...
    while queue.qsize():
        ...
        next_directions = []
        if cur_item.consecutive >= 4:
            next_directions.append(cur_item.direction.turn_left())
            next_directions.append(cur_item.direction.turn_right())
        if cur_item.consecutive < 10:
    ...
    for item, heat_loss in heat_losses.items():
        if not (
            item.coord.i == n_rows - 1
            and item.coord.j == n_cols - 1
            and item.consecutive >= 4
        ):
            continue
